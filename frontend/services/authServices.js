...
const registerCustomer = async (customerData) => {
    return await api.post("/auth/customer/register", customerData);
};

const registerWorker = async (workerData) => {
    return await api.post("/auth/worker/register", workerData);
};
...
const loginWorker = async (credentials) => {
    return await api.post("/auth/worker/login", credentials);
};
...
const saveAuthData = (data, role) => {
    ...
    localStorage.setItem("nexserve_token", data.token);
...
const getCurrentUser = () => {
    const user = localStorage.getItem("nexserve_user");
